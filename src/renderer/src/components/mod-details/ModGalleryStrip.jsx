...
export default function ModGalleryStrip({ mediaItems, currentIndex, onSelect }) {
  ...
  // Якщо немає медіа або тільки 1 елемент - ховаємо стрічку
    ...
    <div className="h-[120px] w-full bg-[#09090b]/90 border-t border-white/5 backdrop-blur-md z-20 flex items-center relative shrink-0 group/gallery px-4">
      ...
      <div ref={galleryRef} className="flex gap-4 overflow-x-auto h-full items-center w-full scroll-smooth px-6" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {mediaItems.map((item, idx) => {
          ...
          // --- АДАПТАЦІЯ ДАНИХ ПІД СТАРИЙ ДИЗАЙН ---
          // В новій структурі у нас є 'layers'. 
          // Шар 0 - це передній план (source). 
          // Останній шар - це фон (backgroundSource), якщо шарів > 1.
          const layers = item.layers || []
          
          // Знаходимо переднє фото (або thumbnail для відео)
          const source = isVideo ? item.thumbnail : (layers[0]?.src || item.thumbnail)
          
          // Знаходимо фон (якщо це картинка і є більше 1 шару)
          // Беремо останній шар (найглибший) як фон
          const backgroundSource = (!isVideo && layers.length > 1) ? layers[layers.length - 1].src : null
          
          const hasDualLayer = !!backgroundSource
          // ------------------------------------------
            ...
            >
              {isVideo ? (
                <div className="w-full h-full relative group">
                  {source ? (
                    <img src={source} className="w-full h-full object-cover opacity-80" alt="video thumbnail" />
                  ) : (
                ...
                // ЛОГІКА ПОДВІЙНОГО ШАРУ (Parallax у мініатюрі)
                hasDualLayer ? (
                   <div className="w-full h-full relative bg-black">
                      {/* Шар 1: Фон (заповнює все) */}
                      <img src={backgroundSource} className="absolute inset-0 w-full h-full object-cover opacity-80 blur-[1px]" alt="" />
                      {/* Шар 2: Основне фото (поверх, contain щоб було видно деталі) */}
                      <img src={source} className="absolute inset-0 w-full h-full object-contain z-10 shadow-lg" alt="" />
                   </div>
                ) : (
                   // Звичайне фото
